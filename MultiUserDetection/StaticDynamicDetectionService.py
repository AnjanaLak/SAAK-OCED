...
def predict_human_movements(candidateID, examinationID):
    ...
    frames_dir = '../Frames/'
    os.chdir(os.getcwd() + '/MultiUserDetection/')
    ...
    j = 0
    print("the current working directory before while is =>" + os.getcwd())
    while True:
        j += 1
        try:
            ...
            # print(ret)
            # if ret == False:
            #     # If frame is read not correctly
            #     break
            img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # convert bgr to rbg format to identify as original image
            img = cv2.resize(img, (320, 320))  # resize the captued frame
            img = img.astype(np.float32)  # Cast a numpy array to a float32
            img = np.expand_dims(img, 0)  # expand the shape of an array insert a new axis
            img = img / 255  # normalizing
            class_names = [c.strip() for c in
                           open("classes.TXT").readlines()]  # Return a copy of the string with leading a
            # nd trailing whitespace removed.
            ...
            if count == 0:
                ...
                print('start time ' + 'Duration: {}'.format(start_time))
